...
description: "SHOW MATERIALIZED VIEWS は、すべてまたは特定の非同期マテリアライズドビューを表示します。"
---
...
> 上記の出力例は省略されています。`SHOW MATERIALIZED VIEWS` は `warehouse`、`refresh_mode`、`refresh_trigger`、`refresh_policy`、`resource_group` の各列も返します。
