...
| QUERY_REWRITE_STATUS                 | 物化视图的查询改写状态。有效值：`VALID`、`INVALID` 和 `UNKNOWN`。该状态的原因请参见 `QUERY_REWRITE_STATUS_REASON`。 |
| CREATOR                              | 物化视图的创建者。                               |
| LAST_REFRESH_PROCESS_TIME            | 最近一次刷新任务的处理时间。                     |
| LAST_REFRESH_JOB_ID                  | 最近一次刷新任务的作业 ID。                      |
| LAST_REFRESH_TIME                    | 物化视图已反映基表更新的最新时间。               |
| WAREHOUSE                            | 异步物化视图执行刷新任务所使用的 warehouse 名称。对于同步（rollup）物化视图为空。 |
...
| RESOURCE_GROUP                       | 物化视图刷新任务所使用的资源组（来自物化视图的 `session.resource_group` 属性）。未设置时默认为 `default_mv_wg`。 |
