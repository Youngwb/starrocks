...
description: "SHOW MATERIALIZED VIEWS 展示所有或某个指定的异步物化视图。"
---
...
> 上述示例输出经过省略。`SHOW MATERIALIZED VIEWS` 还会返回 `warehouse`、`refresh_mode`、`refresh_trigger`、`refresh_policy` 和 `resource_group` 列。
