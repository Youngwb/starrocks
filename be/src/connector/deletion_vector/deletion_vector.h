...
struct DeletionVectorBuildStats {
    ...
    int64_t bitmap_to_array_ns = 0;
    int64_t build_skip_set_ns = 0;
};
...
public:
    ...
    Status deserialized_inline_dv(std::string& encoded_bitmap_data, std::set<int64_t>* need_skip_rowids);
...
private:
    ...
    Status deserialized_deletion_vector(uint32_t magic_number, std::unique_ptr<char[]> serialized_dv,
                                        int64_t serialized_bitmap_length, std::set<int64_t>* need_skip_rowids);
