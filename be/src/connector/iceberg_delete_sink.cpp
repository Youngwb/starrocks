...
Status IcebergDeleteSink::add(const ChunkPtr& chunk) {
    ...
    LOG(INFO) << "IcebergDeleteSink: partition=" << partition;
...
// IcebergDeleteSinkProvider implementation
StatusOr<std::unique_ptr<ConnectorChunkSink>> IcebergDeleteSinkProvider::create_chunk_sink(
    ...
    return std::make_unique<IcebergDeleteSink>(ctx->partition_column_names, ctx->transform_exprs,
