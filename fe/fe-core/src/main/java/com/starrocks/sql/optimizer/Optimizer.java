...
public class Optimizer {
    ...
    private OptExpression extractNthPlanImpl(PhysicalPropertySet requiredProperty, Group group, int nthExecPlan) {
        ...
        for (GroupExpression physicalExpression : group.getSatisfyRequiredGroupExpressions(requiredProperty)) {
            if (planCountOfKGroupExpression + physicalExpression.getPlanCountRootedThis() >= nthExecPlan) {
                chooseGroupExpression = physicalExpression;
                break;
            }
            planCountOfKGroupExpression += physicalExpression.getPlanCountRootedThis();
        }

        Preconditions.checkState(chooseGroupExpression != null);
        OptExpression chooseExpression = null;
        ...
        chooseExpression = OptExpression.create(chooseGroupExpression.getOp(), childPlans);
    ...
    private int computeNthOfChildGroupImpl(int childIndex, int childrenSize, int localNth,
        ...
        if (childIndex + 1 == childrenSize) {
            ...
            // The extra 1 is because nTh starts at 1
            int result = subsequentChildGroupRank.get() %
                    computeKChildrenPlanCount(childIndex, childGroupExprCountList) + 1;
    ...
    // Calculate the valid plan count for the entire group
    private int countGroupValidPlan(Group group, PhysicalPropertySet requiredProperty) {
        ...
        for (GroupExpression physicalExpression : group.getSatisfyRequiredGroupExpressions(requiredProperty)) {
            if (physicalExpression.getInputs().isEmpty()) {
                ...
                physicalExpression.setPlanCountRootedThis(1);
            ...
            groupPlanCount += physicalExpression.getPlanCountRootedThis();
        }
        return groupPlanCount;
    }

    // Calculate the valid plan count for the group expression
    private int countGroupExpressionValidPlan(GroupExpression groupExpression, PhysicalPropertySet requiredProperty) {
        ...
        groupExpression.setPlanCountRootedThis(groupExpressionPlanCount);
